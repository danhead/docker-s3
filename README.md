...
## Backup
Launch `docker-s3` container with the following flags:

```
$ docker run --rm \
--env-file env.txt \
--volumes-from mysql \
--name docker-s3 suburbanme/docker-s3:latest
```
...
`docker-s3` will use your AWS credentials to create a new bucket with name as per the environment variable `S3_BUCKET_NAME`, or if not defined, using the default name `docker-backups.example.com`. The paths in `PATHS_TO_BACKUP` will be tarballed, gzipped, time-stamped and uploaded to the S3 bucket.

### Regular backup with CRON

If you define the environment variable `CRON_TIME`, the container will run the backup script on the interval defined. 
